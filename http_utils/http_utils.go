...
func (this *HttpClient) SendPostRequest(path string, body io.Reader, params map[string]string, headers map[string]string) (*http.Response, error) {
	req, err := this.GetRequestWithHeader("POST", path, body, params, headers)
	if err != nil {
		return nil, fmt.Errorf("GetRequestWithHeader returned an error: %v", err)
	}
